...
def convert_task_to_api_format(task_json):
    ...
    return {
        ...
        "prompt": normalize_prompt(task.get("prompt", "")),
...
def main():
    if len(sys.argv) < 2:
        print("用法: python3 scripts/submit_tasks.py /path/to/project [filename_contains]")
        sys.exit(1)

    project_dir = sys.argv[1]
    filename_contains = sys.argv[2] if len(sys.argv) >= 3 else None
    ...
    for filename in sorted(os.listdir(project_dir)):
        ...
        print(f"   Prompt length: {len(api_data['prompt'])}")
